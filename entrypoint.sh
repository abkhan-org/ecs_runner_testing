...
_CHECKING_INTERVAL=${JOB_CHECKING_INTERVAL:-2}
...
if ! [ -z $AUTO_TERMINATION ]; then
 # Echo the cron job command into the crontab file
  echo "*/$_CHECKING_INTERVAL * * * * /bin/bash /auto_termination.sh >/tmp/auto_termination.log 2>&1" | crontab -
  service cron start
fi
